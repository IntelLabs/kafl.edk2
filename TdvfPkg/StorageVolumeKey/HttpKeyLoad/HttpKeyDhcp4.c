/** @file
  Functions implementation related with DHCPv4 for HTTP key driver.
...
//
UINT8 mInterestedDhcp4Tags[HTTP_KEY_DHCP4_TAG_INDEX_MAX] = {
  DHCP4_TAG_OVERLOAD,
  DHCP4_TAG_MSG_TYPE,
  DHCP4_TAG_SERVER_ID,
...
/**
  ...
  @param[in]  Private             Pointer to HTTP key driver private data.
  ...
  )
{
  ...
  OptEnt.Para               = (HTTP_KEY_DHCP4_OPTION_PARA *) OptList[Index]->Data;
  ...
  OptList[Index]->Length  = (UINT8) sizeof (HTTP_KEY_DHCP4_OPTION_UUID);
  OptEnt.Uuid             = (HTTP_KEY_DHCP4_OPTION_UUID *) OptList[Index]->Data;
  OptEnt.Uuid->Type       = 0;
  ...
  OptList[Index]->Length  = (UINT8) sizeof (HTTP_KEY_DHCP4_OPTION_UNDI);
  OptEnt.Undi             = (HTTP_KEY_DHCP4_OPTION_UNDI *) OptList[Index]->Data;
  ...
  OptList[Index]->Length  = (UINT8) sizeof (HTTP_KEY_DHCP4_OPTION_ARCH);
  OptEnt.Arch             = (HTTP_KEY_DHCP4_OPTION_ARCH *) OptList[Index]->Data;
  Value                   = HTONS (EFI_HTTP_KEY_CLIENT_SYSTEM_ARCHITECTURE);
  ...
  OptList[Index]->Length  = (UINT8) sizeof (HTTP_KEY_DHCP4_OPTION_CLID);
  OptEnt.Clid             = (HTTP_KEY_DHCP4_OPTION_CLID *) OptList[Index]->Data;
  CopyMem (
    OptEnt.Clid,
    DEFAULT_CLASS_ID_DATA,
    sizeof (HTTP_KEY_DHCP4_OPTION_CLID)
    );
  HttpKeyUintnToAscDecWithFormat (
    EFI_HTTP_KEY_CLIENT_SYSTEM_ARCHITECTURE,
  ...
  if (Private->Nii != NULL) {
    ...
    HttpKeyUintnToAscDecWithFormat (Private->Nii->MajorVer, OptEnt.Clid->UndiMajor, sizeof (OptEnt.Clid->UndiMajor));
    HttpKeyUintnToAscDecWithFormat (Private->Nii->MinorVer, OptEnt.Clid->UndiMinor, sizeof (OptEnt.Clid->UndiMinor));
  }
...
**/
EFI_STATUS
HttpKeyParseDhcp4Packet (
  IN HTTP_KEY_DHCP4_PACKET_CACHE    *Cache4
  )
{
  ...
  //
  for (Index = 0; Index < HTTP_KEY_DHCP4_TAG_INDEX_MAX; Index++) {
    Options[Index] = HttpKeyParseDhcp4Options (
                       Offer->Dhcp4.Option,
  ...
  //
  Option = Options[HTTP_KEY_DHCP4_TAG_INDEX_CLASS_ID];
  ...
  //
  Option = Options[HTTP_KEY_DHCP4_TAG_INDEX_DNS_SERVER];
  if (Option != NULL) {
    IsDnsOffer = TRUE;
  }
...
**/
EFI_STATUS
HttpKeyCacheDhcp4Offer (
  IN HTTP_KEY_PRIVATE_DATA  *Private,
  IN EFI_DHCP4_PACKET        *RcvdOffer
  )
{
  HTTP_KEY_DHCP4_PACKET_CACHE  *Cache4;
  EFI_DHCP4_PACKET              *Offer;
  HTTP_KEY_OFFER_TYPE          OfferType;
  EFI_STATUS                    Status;

  ASSERT (Private->OfferNum < HTTP_KEY_OFFER_MAX_NUM);
  ...
  //
  Status = HttpKeyCacheDhcp4Packet (Offer, RcvdOffer);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  //
  if (EFI_ERROR (HttpKeyParseDhcp4Packet (Cache4))) {
    return EFI_ABORTED;
  }
  ...
  ASSERT (Private->OfferCount[OfferType] < HTTP_KEY_OFFER_MAX_NUM);
  ...
  )
{
  ...
  //
  MaxMsgSize = HttpKeyParseDhcp4Options (
                 Packet->Dhcp4.Option,
                 GET_OPTION_BUFFER_LEN (Packet),
                 DHCP4_TAG_MAXMSG
                 );
  if (MaxMsgSize != NULL) {
    Value = HTONS (HTTP_KEY_DHCP4_PACKET_MAX_SIZE);
  ...
  //
  if (Private->HttpKeyCallback != NULL && Dhcp4Event != Dhcp4SelectOffer) {
  ...
  case Dhcp4RcvdOffer:
    ...
    if (Packet->Length > HTTP_KEY_DHCP4_PACKET_MAX_SIZE) {
      //
      // Ignore the incoming packets which exceed the maximum length.
      //
      break;
    }
    if (Private->OfferNum < HTTP_KEY_OFFER_MAX_NUM) {
      //
      // Cache the DHCPv4 offers to OfferBuffer[] for select later, and record
      // the OfferIndex and OfferCount.
      // If error happens, just ignore this packet and continue to wait more offer.
      //
      HttpKeyCacheDhcp4Offer (Private, Packet);
    }
    break;
  ...
  case Dhcp4SelectOffer:
    ...
    // and SelectProxyType.
    //
    HttpKeySelectDhcpOffer (Private);
...
/**
  ...
  @param[in]  Private             The pointer to HTTP_KEY_PRIVATE_DATA.
...
/**
  Start the D.O.R.A DHCPv4 process to acquire the IPv4 address and other Http key information.
  ...
  )
{
  ...
  UINT32                       OptCount;
  EFI_DHCP4_PACKET_OPTION      *OptList[HTTP_KEY_DHCP4_OPTION_MAX_NUM];
  UINT8                        Buffer[HTTP_KEY_DHCP4_OPTION_MAX_SIZE];
  ...
  Status = HttpKeySetIp4Policy (Private);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  //
  OptCount = HttpKeyBuildDhcp4Options (Private, OptList, Buffer);
  ASSERT (OptCount > 0);

  ZeroMem (&Config, sizeof(Config));
  Config.OptionCount      = OptCount;
  Config.OptionList       = OptList;
  Config.Dhcp4Callback    = HttpKeyDhcp4CallBack;
  Config.CallbackContext  = Private;
  Config.DiscoverTryCount = HTTP_KEY_DHCP_RETRIES;
  Config.DiscoverTimeout  = mHttpDhcpTimeout;

  //
  // Configure the DHCPv4 instance for HTTP key.
  //
  ...
  Status = HttpKeyRegisterIp4Gateway (Private);
  if (EFI_ERROR (Status)) {
    goto ON_EXIT;
  }

  AsciiPrint ("\n  Station IP address is ");
  HttpKeyShowIp4Addr (&Private->StationIp.v4);
  AsciiPrint ("\n");
