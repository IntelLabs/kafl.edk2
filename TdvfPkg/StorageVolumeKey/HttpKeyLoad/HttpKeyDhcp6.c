/** @file
  Functions implementation related with DHCPv6 for HTTP key driver.
...
/**
  ...
  @param[in]  Private             The pointer to HTTP KEY driver private data.
...
**/
UINT32
HttpKeyBuildDhcp6Options (
  IN  HTTP_KEY_PRIVATE_DATA       *Private,
  OUT EFI_DHCP6_PACKET_OPTION      **OptList,
  IN  UINT8                        *Buffer
  )
{
  HTTP_KEY_DHCP6_OPTION_ENTRY     OptEnt;
  UINT16                           Value;
  UINT32                           Index;
  ...
  OptEnt.Oro                 = (HTTP_KEY_DHCP6_OPTION_ORO *) OptList[Index]->Data;
  ...
  OptEnt.Undi                = (HTTP_KEY_DHCP6_OPTION_UNDI *) OptList[Index]->Data;
  ...
  OptList[Index]->OpLen      = HTONS ((UINT16) sizeof (HTTP_KEY_DHCP6_OPTION_ARCH));
  OptEnt.Arch                = (HTTP_KEY_DHCP6_OPTION_ARCH *) OptList[Index]->Data;
  ...
  OptList[Index]->OpLen        = HTONS ((UINT16) sizeof (HTTP_KEY_DHCP6_OPTION_VENDOR_CLASS));
  OptEnt.VendorClass           = (HTTP_KEY_DHCP6_OPTION_VENDOR_CLASS *) OptList[Index]->Data;
  OptEnt.VendorClass->Vendor   = HTONL (HTTP_KEY_DHCP6_ENTERPRISE_NUM);
  OptEnt.VendorClass->ClassLen = HTONS ((UINT16) sizeof (HTTP_KEY_CLASS_ID));
  CopyMem (
    &OptEnt.VendorClass->ClassId,
    DEFAULT_CLASS_ID_DATA,
    sizeof (HTTP_KEY_CLASS_ID)
    );
...
**/
EFI_STATUS
HttpKeyParseDhcp6Packet (
  IN  HTTP_KEY_DHCP6_PACKET_CACHE    *Cache6
  )
{
  ...
  //
  Option = Options[HTTP_KEY_DHCP6_IDX_IA_NA];
  if (Option != NULL) {
    Option = HttpKeyParseDhcp6Options (
               Option->Data + 12,
               NTOHS (Option->OpLen),
  ...
  //
  Option = Options[HTTP_KEY_DHCP6_IDX_VENDOR_CLASS];
  ...
  //
  Option = Options[HTTP_KEY_DHCP6_IDX_DNS_SERVER];
  if (Option != NULL) {
    IsDnsOffer = TRUE;
  }
...
**/
EFI_STATUS
HttpKeyCacheDhcp6Offer (
  IN HTTP_KEY_PRIVATE_DATA  *Private,
  IN EFI_DHCP6_PACKET        *RcvdOffer
  )
{
  HTTP_KEY_DHCP6_PACKET_CACHE   *Cache6;
  EFI_DHCP6_PACKET               *Offer;
  HTTP_KEY_OFFER_TYPE           OfferType;
  EFI_STATUS                     Status;
  ...
  //
  Status = HttpKeyCacheDhcp6Packet(Offer, RcvdOffer);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  //
  if (EFI_ERROR (HttpKeyParseDhcp6Packet (Cache6))) {
    return EFI_ABORTED;
  }
  ...
  )
{
  ...
  Status = EFI_SUCCESS;
  if (Private->HttpKeyCallback != NULL && Dhcp6Event != Dhcp6SelectAdvertise) {
  ...
  case Dhcp6RcvdAdvertise:
    ...
    if (Packet->Length > HTTP_KEY_DHCP6_PACKET_MAX_SIZE) {
      //
    ...
    if (Private->OfferNum < HTTP_KEY_OFFER_MAX_NUM) {
      ...
      //
      HttpKeyCacheDhcp6Offer (Private, Packet);
    }
    break;
...
)
{
  ...
  Ip6CfgData.HopLimit            = HTTP_KEY_DEFAULT_HOPLIMIT;
  Ip6CfgData.ReceiveTimeout      = HTTP_KEY_DEFAULT_LIFETIME;
  Ip6CfgData.TransmitTimeout     = HTTP_KEY_DEFAULT_LIFETIME;
  ...
  //
  Status = HttpKeyCheckRouteTable (Private, HTTP_KEY_IP6_ROUTE_TABLE_TIMEOUT, &GatewayAddr);
  if (EFI_ERROR (Status)) {
...
/**
  Start the S.A.R.R DHCPv6 process to acquire the IPv6 address and other Http key information.
  ...
  )
{
  ...
  EFI_DHCP6_PACKET_OPTION          *OptList[HTTP_KEY_DHCP6_OPTION_MAX_NUM];
  UINT32                           OptCount;
  UINT8                            Buffer[HTTP_KEY_DHCP6_OPTION_MAX_SIZE];
  EFI_STATUS                       Status;
  ...
  //
  OptCount = HttpKeyBuildDhcp6Options (Private, OptList, Buffer);
  ASSERT (OptCount >0);
  ...
  Config.Dhcp6Callback         = HttpKeyDhcp6CallBack;
  ...
  //
  // Configure the DHCPv6 instance for HTTP key.
  //
  ...
  HttpKeyShowIp6Addr (&Private->StationIp.v6);
  AsciiPrint ("\n");
