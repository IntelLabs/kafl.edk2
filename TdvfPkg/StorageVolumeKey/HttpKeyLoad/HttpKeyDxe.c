/** @file
  Driver Binding functions implementation for UEFI HTTP key.
...
///
EFI_DRIVER_BINDING_PROTOCOL gHttpKeyIp4DxeDriverBinding = {
  HttpKeyIp4DxeDriverBindingSupported,
  HttpKeyIp4DxeDriverBindingStart,
  HttpKeyIp4DxeDriverBindingStop,
  HTTP_KEY_DXE_VERSION,
  NULL,
  NULL
};

EFI_DRIVER_BINDING_PROTOCOL gHttpKeyIp6DxeDriverBinding = {
...
/**
  ...
  @param[in]   Private           Pointer to HTTP_KEY_PRIVATE_DATA.
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    Private = HTTP_KEY_PRIVATE_DATA_FROM_ID(Id);
  } else {
    FirstStart = TRUE;
    ...
    //
    Private = AllocateZeroPool (sizeof (HTTP_KEY_PRIVATE_DATA));
    if (Private == NULL) {
      return EFI_OUT_OF_RESOURCES;
    }
    Private->Signature = HTTP_KEY_PRIVATE_DATA_SIGNATURE;
  ...
  Private->Ip4Nic = AllocateZeroPool (sizeof (HTTP_KEY_VIRTUAL_NIC));
  if (Private->Ip4Nic == NULL) {
  ...
  Private->Ip4Nic->Signature   = HTTP_KEY_VIRTUAL_NIC_SIGNATURE;
  ...
  //
  // Create a child handle for the HTTP key and install DevPath and Load file protocol on it.
  //
  CopyMem (&Private->Ip4Nic->LoadKey, &gHttpKeyDxeLoadKey, sizeof (LOAD_KEY_PROTOCOL));
  ...
  // real NIC handle and the HTTP key Ipv4 NIC handle.
  //
  Status = gBS->OpenProtocol (
                  ControllerHandle,
                  &gEfiCallerIdGuid,
...
ON_ERROR:
  if (Private != NULL) {
    ...
    HttpKeyDestroyIp4Children (This, Private);
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    //
    NicHandle = HttpKeyGetNicByIp4Children (ControllerHandle);
    if (NicHandle == NULL) {
      return EFI_SUCCESS;
    }
    ...
    Private = HTTP_KEY_PRIVATE_DATA_FROM_LOADKEY (LoadKey);
    NicHandle = Private->Controller;
  }

  //
  // Disable the HTTP key function.
  //
  ...
  )
{
  ...
  //
  Status = HttpKeyCheckIpv6Support (Private, &Ipv6Available);
  if (EFI_ERROR (Status)) {
    //
  ...
  Private->Ip6Nic = AllocateZeroPool (sizeof (HTTP_KEY_VIRTUAL_NIC));
  if (Private->Ip6Nic == NULL) {
    Status = EFI_OUT_OF_RESOURCES;
    goto ON_ERROR;
  }
  ...
  Private->Ip6Nic->Signature   = HTTP_KEY_VIRTUAL_NIC_SIGNATURE;
  ...
  // Create a child handle for the HTTP key and install DevPath and Load file protocol on it.
  //
  CopyMem (&Private->Ip6Nic->LoadKey, &gHttpKeyDxeLoadKey, sizeof (Private->LoadKey));
  Status = gBS->InstallMultipleProtocolInterfaces (
                  &Private->Ip6Nic->Controller,
                  &gLoadKeyProtocolGuid,
                  &Private->Ip6Nic->LoadKey,
  ...
  //
  // Open the Caller Id child to setup a parent-child relationship between
  // real NIC handle and the HTTP key child handle.
  //
  Status = gBS->OpenProtocol (
                  ControllerHandle,
                  &gEfiCallerIdGuid,
...
ON_ERROR:
  if (Private != NULL) {
    ...
    HttpKeyDestroyIp6Children(This, Private);
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    // If failed, try to find the NIC handle for this controller.
    //
    NicHandle = HttpKeyGetNicByIp6Children (ControllerHandle);
    if (NicHandle == NULL) {
      return EFI_SUCCESS;
    }
  ...
  )
{
  ...
  Status = EfiLibInstallDriverBindingComponentName2 (
             ImageHandle,
             SystemTable,
             &gHttpKeyIp4DxeDriverBinding,
             ImageHandle,
             &gHttpKeyDxeComponentName,
             &gHttpKeyDxeComponentName2
             );
  if (EFI_ERROR (Status)) {
    return Status;
  }
