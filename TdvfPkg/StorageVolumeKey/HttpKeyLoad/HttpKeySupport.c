/** @file
  Support functions implementation for UEFI HTTP key driver.
  ...
  )
{
  ...
  //
  Status = gBS->SetTimer (HttpIo->TimeoutEvent, TimerRelative, HTTP_KEY_RESPONSE_TIMEOUT * TICKS_PER_MS);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  )
{
  ...
  if ((AsciiStrnCmp (Uri, "http://", 7) != 0) && (AsciiStrnCmp (Uri, "https://", 8) != 0)) {
    DEBUG ((EFI_D_ERROR, "HttpKeyCheckUriScheme: Invalid Uri.\n"));
    return EFI_INVALID_PARAMETER;
  }
  ...
  if (!PcdGetBool (PcdAllowHttpConnections) && (AsciiStrnCmp (Uri, "http://", 7) == 0)) {
    DEBUG ((EFI_D_ERROR, "HttpKeyCheckUriScheme: HTTP is disabled.\n"));
  ...
  )
{
  ...
  UriStrLength = StrLen(Temp);
  Uri = AllocatePool (UriStrLength + 1);
  if (Uri == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }
  UnicodeStrToAsciiStrS (Temp, Uri, UriStrLength+1);
  *UriAddress = Uri;
  return EFI_SUCCESS;
}
...
**/
BOOLEAN
HttpKeyIsHttpRedirectStatusCode (
