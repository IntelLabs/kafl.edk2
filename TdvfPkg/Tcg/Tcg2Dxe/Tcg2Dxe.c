...
typedef struct{
  ...
  UINT64                      Laml;
  UINT64                      Lasa;
}EFI_TDVF_EVENTLOG_ACPI_TABLE;
#pragma pack()

#define EFI_TDVF_EVENTLOG_ACPI_TABLE_SIGNATURE  SIGNATURE_32('T', 'D', 'E', 'L')
#define EFI_TDVF_EVENTLOG_ACPIT_TABLE_REVISION  1

EFI_TDVF_EVENTLOG_ACPI_TABLE mTdvfEventlogAcpiTemplate = {
  {
    EFI_TDVF_EVENTLOG_ACPI_TABLE_SIGNATURE,
    sizeof(mTdvfEventlogAcpiTemplate),
    EFI_TDVF_EVENTLOG_ACPIT_TABLE_REVISION,
    //
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "    EventSize - 0x%08x, EventBuffer - %p\n", EventSize, EventBuffer));
  ...
  )
{
  ...
  //
  // map the PCRIndex to MRTD/RTMR index
  ...
  )
{
  ...
  if ((Flags & PE_COFF_IMAGE) != 0) {
    // measure Pe Image and then extend to TPM(RTMR)
    ...
    //
    // do a hash operation on DataToHash
    // then extend the hash result to TPM(RTMR)
    ...
    if ((mTcgDxeData.BsCap.SupportedEventLogs & mTcg2EventInfo[Index].LogFormat) != 0) {
      ...
      while (!EFI_ERROR (Status) && (GuidHob.Raw = GetNextGuidHob (mTcg2EventInfo[Index].EventGuid, GuidHob.Raw)) != NULL) {
        ...
        if(mTcg2EventInfo[Index].LogFormat == EFI_TCG2_EVENT_LOG_FORMAT_TCG_2) {
          ...
          DEBUG((DEBUG_INFO, "DigestListBinSize=0x%x, EventSize=0x%x, HashDataLen=0x%x\n", 
                 DigestListBinSize, EventSize,  TdxEvent->HashDataLen));
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "TDVF>>>> Tcg2Dxe: MeasureVariable (Pcr - %x, EventType - %x, ", (UINTN)PCRIndex, (UINTN)EventType));
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "TDVF>>>> Tcg2Dxe Measure Data when ReadyToBoot\n"));
  //
  ...
  )
{
  ...
  mTdvfEventlogAcpiTemplate.Laml = (UINT64)PcdGet32(PcdTpm2AcpiTableLaml);
  mTdvfEventlogAcpiTemplate.Lasa = PcdGet64(PcdTpm2AcpiTableLasa);
  CopyMem (mTdvfEventlogAcpiTemplate.Header.OemId, PcdGetPtr (PcdAcpiDefaultOemId), sizeof (mTdvfEventlogAcpiTemplate.Header.OemId));
  OemTableId = PcdGet64 (PcdAcpiDefaultOemTableId);
  CopyMem (&mTdvfEventlogAcpiTemplate.Header.OemTableId, &OemTableId, sizeof (UINT64));
  mTdvfEventlogAcpiTemplate.Header.OemRevision      = PcdGet32 (PcdAcpiDefaultOemRevision);
  mTdvfEventlogAcpiTemplate.Header.CreatorId        = PcdGet32 (PcdAcpiDefaultCreatorId);
  mTdvfEventlogAcpiTemplate.Header.CreatorRevision  = PcdGet32 (PcdAcpiDefaultCreatorRevision);

  //
  // Construct ACPI Table
  Status = AcpiTable->InstallAcpiTable(
                        AcpiTable,
                        &mTdvfEventlogAcpiTemplate,
                        mTdvfEventlogAcpiTemplate.Header.Length,
                        &TableKey
                        );
  ASSERT_EFI_ERROR (Status);
  ...
  DEBUG((DEBUG_INFO, "TDVF Eventlog ACPI Table is measured and logged\n"));
  ...
  )
{
  ...
  // For TDVF, we only support SHA384
  //
  mTcgDxeData.BsCap.HashAlgorithmBitmap = HASH_ALG_SHA384 & PcdGet32 (PcdTcg2HashAlgorithmBitmap);
  mTcgDxeData.BsCap.ActivePcrBanks = HASH_ALG_SHA384 & PcdGet32 (PcdTcg2HashAlgorithmBitmap);
  NumberOfPCRBanks = mTcgDxeData.BsCap.ActivePcrBanks == 0 ? 0 : 1;
  ...
  if( Status == EFI_SUCCESS && mTcgDxeData.BsCap.TPMPresentFlag){
    // 
