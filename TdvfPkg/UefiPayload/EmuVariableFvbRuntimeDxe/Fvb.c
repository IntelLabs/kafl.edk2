...
/**
  ...
  @param[in] TdvfCfvBase - A pointer to CFV header
  @param[in] TdvfCfvSize - CFV data size
  ...
  ) 
{
  ...
  if (TdvfCfvBase == NULL) {
    DEBUG ((DEBUG_ERROR, "TDVF CFV: CFV pointer is NULL\n"));
    return FALSE;
  }

  if (TdvfCfvSize > EMU_FVB_SIZE) {
    DEBUG ((DEBUG_ERROR, "TDVF CFV: CFV size larger than EMU_FVB_SIZE\n"));
    return FALSE;
  }
  ...
  if ((!IsZeroBuffer(CfvFvHeader->ZeroVector, 16)) ||
    ...
    DEBUG ((DEBUG_ERROR, "TDVF CFV: Basic FV headers were invalid\n"));
    return FALSE;
  }
  ...
  if (Checksum != 0) {
    DEBUG ((DEBUG_ERROR, "TDVF CFV: FV checksum was invalid\n"));
    return FALSE;
  }
  ...
  //  
  CfvVariableStoreHeader = (VARIABLE_STORE_HEADER *) (TdvfCfvBase + CfvFvHeader->HeaderLength);
  if ((!CompareGuid(&VarStoreHdrGUID, &CfvVariableStoreHeader->Signature)) ||
    ...
    DEBUG ((DEBUG_ERROR, "TDVF CFV: Variable Store header was invalid\n"));
    return FALSE;
  }
  ...
  //  
  VariableBase = (UINTN)TdvfCfvBase + CfvFvHeader->HeaderLength + sizeof(VARIABLE_STORE_HEADER); 
  ...
  if ((VariableBase + VariableOffset) != ((UINTN)TdvfCfvBase + TdvfCfvSize)) {
    DEBUG ((DEBUG_ERROR, "TDVF CFV: Top address of CFV does not match\n"));
    return FALSE;
  }
  ...
  )
{
  ...
  //
  // Copy the content from Configuration FV in TDVF project
  //
  if(PcdGetBool(PcdTdvfInitializeByCfv)){
    DEBUG((DEBUG_INFO, "Copy the content from Configuration FV\n"));
    CfvBase = (UINT8*)(UINTN)PcdGet64(PcdTdvfCfvBase);
    CfvSize = (UINT32)PcdGet64(PcdTdvfCfvSize);
    
    if(ValidateTdvfCfv(CfvBase, CfvSize)) {
