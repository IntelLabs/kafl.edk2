/** @file
  The library instance provides security service of Tdvf measure boot.
...
/**
  Measure GPT table data into TCG2 log.
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    //
    // Td Tcg2 protocol is not installed.
    // Don't do any measurement, and directly return EFI_SUCCESS.
    //
    DEBUG ((EFI_D_VERBOSE, "Tdx DxeTpm2MeasureBootHandler -  %r\n", Status));
    return EFI_SUCCESS;
  }
      ...
      if (DevicePathType (DevicePathNode) == MEDIA_DEVICE_PATH &&
        ...
        if (((HARDDRIVE_DEVICE_PATH *) DevicePathNode)->MBRType == MBR_TYPE_EFI_PARTITION_TABLE_HEADER &&
          ...
          if (!EFI_ERROR (Status)) {
            ...
            DEBUG ((EFI_D_INFO, "Tdx DxeTpm2MeasureBootHandler - Tcg2MeasureGptTable - %r\n", Status));
            if (!EFI_ERROR (Status)) {
              //
              // GPT disk check done.
              //
  ...
  if ((!ApplicationRequired) ||
    ...
    DEBUG ((EFI_D_INFO, "Tdx DxeTpm2MeasureBootHandler - Tcg2MeasurePeImage - %r\n", Status));
  }
...
Finish:
  ...
  DEBUG ((EFI_D_INFO, "Tdx DxeTpm2MeasureBootHandler - %r\n", Status));

  return Status;
}

/**
  Register the security handler to provide Tdvf measure boot service.
